...
if (DO_SBOM === "true") {
    logger.info("Generating SBOM for Torizon RAC ...")

    // get the version
    // the version from rac comes from the recipe json file

    let rac_version = META.version
    ...
    _cdx_sbom.components.add(
        new CDX.Models.Component(
            ...
            {
                version: `${rac_version}`,
                cpe: `cpe:2.3:*:*:rac:${rac_version}:*:*:*:*:*:*:*`,
                purl: PackageURL.fromString(`pkg:generic/rac@${rac_version}`)
            }
        )
    )
    ...
    logger.success("SBOM generation for Torizon RAC completed successfully.")
}
