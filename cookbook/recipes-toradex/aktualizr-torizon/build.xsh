...
# set the right compiler path for arch lib
$COMPILER_ARCH = $(gcc -dumpmachine)
$DEB_HOST_MULTIARCH = $(dpkg-architecture -qDEB_HOST_MULTIARCH)
$DEB_VERSION_UPSTREAM = "2025.01"


os.chdir(f"{_BUILD_ROOT}/aktualizr-torizon")


# check if the .deb already exists, if so only return
if os.path.exists(f"{_BUILD_ROOT}/aktualizr-torizon/build/aktualizr.deb"):
    print(
        "building target garage tools, already built, ok",
        color=Color.WHITE,
...
print(
    f"Building target aktualizr version [{_version_str}] with uptane-sign [{os.getenv('UPTANE_SIGN_VER')}]"
)
...
print(
    "building target garage tools, ok",
    color=Color.WHITE,
    bg_color=BgColor.GREEN
)
