...
print(
    "Fetching aktualizr to build target garage tools ...",
    color=Color.WHITE,
...
if not os.path.exists(f"{_BUILD_ROOT}/aktualizr-torizon"):
    os.chdir(f"{_BUILD_ROOT}")
    git clone @(meta['source']) aktualizr-torizon
...
print(
    "aktualizr to build target garage tools, ok",
    color=Color.WHITE,
    bg_color=BgColor.GREEN
)
