...
if (DO_SBOM === "true") {
    logger.info("Generating SBOM for mplayer ...")
    ...
    let mplayer_version = "unknown"
    mplayer_version = execSync(
        "cat VERSION",
        {
            cwd: app_src_path,
            encoding: "utf-8",
    ...
    _cdx_sbom.components.add(
        new CDX.Models.Component(
            ...
            {
                version: `${mplayer_version}`,
                cpe: `cpe:2.3:*:*:mplayer:${mplayer_version}:*:*:*:*:*:*:*`,
                purl: PackageURL.fromString(`pkg:generic/mplayer@${mplayer_version}`)
            }
        )
    )
    ...
    logger.success("SBOM generation for mplayer completed successfully.")
}
