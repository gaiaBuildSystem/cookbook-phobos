...
if (DO_SBOM === "true") {
    logger.info("Generating SBOM for mars ...")

    // get the version
    // the version from mars comes from the recipe json file

    let mars_version = META.version
    ...
    _cdx_sbom.components.add(
        new CDX.Models.Component(
            ...
            {
                version: `${mars_version}`,
                cpe: `cpe:2.3:*:*:mars:${mars_version}:*:*:*:*:*:*:*`,
                purl: PackageURL.fromString(`pkg:generic/mars@${mars_version}`)
            }
        )
    )
    ...
    logger.success("SBOM generation for mars completed successfully.")
}
